...
| `ReceivedInvoiceEditor.tsx` | Editor de factura recibida: líneas (base + IVA), retención IRPF a nivel factura, `% imputación` (afectación a la actividad, entero 0-100, default 100), categoría libre, adjuntos. |
...
| `AnalyticsView.tsx` | Selector año/trimestre. 4 KPIs (ingresos, gastos, IVA a ingresar/devolver, IRPF retenido). Modelo 303 con desglose por tipo de IVA, base imponible de ventas y de compras (esta última ponderada por el % de imputación) y resultado (a ingresar/devolver). Las facturas en `borrador` no computan fiscalmente. |
...
- `% imputación` (facturas de compra, `imputacionPct`): entero 0-100, ausente = 100%. Pondera cuánto de la factura computa en la analítica fiscal (base, IVA soportado e IRPF soportado), reflejando la afectación parcial a la actividad. NO altera el "Total a pagar" de la factura (se paga el 100%). Normalizado siempre vía `getImputacionPct` (util).
- Modelo 303 = IVA repercutido (ventas) − IVA soportado (compras). Negativo → "A devolver". El card muestra desglose por tipo de IVA + base imponible total de ventas y de compras. La base (y el IVA) de compras se pondera por el `% imputación` de cada factura.
- El Modelo 130 se retiró de la analítica (el titular no está obligado a presentarlo).
